# 4.2 ANALISIS PROSES SISTEM

Analisis proses sistem merupakan tahapan pemodelan sistem menggunakan pendekatan berorientasi objek dengan Unified Modeling Language (UML). Analisis ini bertujuan untuk memberikan gambaran yang jelas dan lengkap mengenai bagaimana sistem akan bekerja, baik dari segi interaksi antar aktor, alur proses, maupun struktur data yang digunakan.

Analisis proses sistem POS DW mencakup beberapa aspek penting yang akan dijelaskan secara detail pada sub-bab berikut:

1. Use Case Diagram untuk mengidentifikasi aktor dan fungsionalitas sistem
2. Deskripsi Use Case untuk menjelaskan secara rinci setiap interaksi aktor dengan sistem
3. Activity Diagram untuk memodelkan alur aktivitas bisnis
4. Sequence Diagram untuk menjelaskan interaksi antar objek dalam sistem secara temporal
5. Class Diagram untuk menggambarkan struktur class dan relasi antar class
6. Analisis Input dan Output untuk mengidentifikasi data yang masuk dan keluar dari sistem
...
![Use Case Diagram](bab-iv/diagrams/usecase-diagram.png)

*Sumber: Hasil Perancangan (2026)*

### Identifikasi Aktor

Berdasarkan hasil analisis kebutuhan sistem, terdapat tiga aktor yang akan berinteraksi dengan sistem POS DW. Berikut adalah penjelasan masing-masing aktor:

**Tabel 4.8 Daftar Aktor Sistem**

| No | Aktor | Deskripsi | Hak Akses Utama |
|----|-------|-----------|-----------------|
| 1 | Admin | Pengguna dengan hak akses penuh terhadap seluruh sistem. Admin bertanggung jawab dalam pengelolaan master data, pengaturan sistem, dan manajemen user | Kelola seluruh modul (master data, transaksi, laporan, user, role, permission, settings) |
| 2 | Kasir | Pengguna yang bertugas melayani transaksi penjualan di kasir. Kasir memiliki akses terbatas pada modul transaksi dan data yang mendukung operasional kasir | Melakukan transaksi POS, cetak struk, lihat riwayat transaksi, update profil |
| 3 | Manager | Pengguna yang bertugas melakukan monitoring dan analisis bisnis. Manager membutuhkan data laporan untuk pengambilan keputusan | Lihat dashboard, generate laporan, export laporan ke PDF/Excel, update profil |
...
**Tabel 4.9 Daftar Use Case**

| Kode | Use Case | Deskripsi | Aktor |
|------|----------|-----------|-------|
| UC-01 | Login | Proses autentikasi user ke dalam sistem menggunakan email dan password | Admin, Kasir, Manager |
| UC-02 | Setup 2FA | Mengaktifkan atau menonaktifkan fitur Two Factor Authentication | Admin, Kasir, Manager |
| UC-03 | Logout | Proses keluar dari sistem dan menghapus session | Admin, Kasir, Manager |
...
| UC-08 | Transaksi Penjualan | Melakukan proses transaksi penjualan dari awal hingga akhir | Kasir, Admin |
| UC-09 | Tambah Item ke Cart | Menambahkan produk ke dalam keranjang belanja | Kasir, Admin |
| UC-10 | Proses Pembayaran | Melakukan kalkulasi dan memproses pembayaran transaksi | Kasir, Admin |
| UC-11 | Cetak Struk | Mencetak struk atau invoice hasil transaksi | Kasir, Admin |
| UC-12 | Lihat Riwayat Transaksi | Melihat daftar transaksi yang telah dilakukan | Admin, Manager, Kasir |
| UC-13 | Laporan Penjualan | Menghasilkan laporan penjualan berdasarkan periode waktu | Admin, Manager |
| UC-14 | Laporan Stok | Menghasilkan laporan stok produk termasuk stok menipis | Admin, Manager |
| UC-15 | Laporan Produk Terlaris | Menghasilkan laporan produk dengan penjualan tertinggi | Admin, Manager |
| UC-16 | Export PDF | Mengekspor laporan ke dalam format PDF | Admin, Manager |
| UC-17 | Export Excel | Mengekspor laporan ke dalam format Excel | Admin, Manager |
...
| UC-22 | Settings Profil | Memperbarui profil pengguna (nama, email) | Admin, Kasir, Manager |
| UC-23 | Ubah Password | Mengganti password akun pengguna | Admin, Kasir, Manager |
...
Use case Transaksi Penjualan merupakan use case utama dalam sistem POS DW yang memungkinkan kasir atau admin untuk melayani transaksi penjualan kepada customer.

**Tabel 4.13 Spesifikasi Use Case Transaksi Penjualan**
...
| **Aktor** | Kasir, Admin |
| **Deskripsi** | Kasir melakukan proses transaksi penjualan produk kepada customer melalui interface Point of Sale (POS), dimulai dari pemilihan produk, input pembayaran, hingga mencetak struk |
...
3. Kasir mencari produk dengan mengetikkan nama atau SKU pada kolom pencarian
4. Sistem menampilkan hasil pencarian secara real-time tanpa perlu reload halaman
5. Kasir memilih produk yang akan dibeli dengan mengklik tombol tambah
...
8. Kasir dapat mengubah quantity produk sesuai kebutuhan (opsional)
9. Sistem menghitung subtotal per item dan total keseluruhan secara real-time
10. Kasir mengulangi langkah 3-9 untuk setiap produk yang akan dibeli (opsional)
11. Kasir memasukkan nama customer
12. Kasir memilih metode pembayaran (Cash/Debit/Credit/E-Wallet)
13. Kasir memasukkan jumlah pembayaran
14. Sistem menghitung dan menampilkan jumlah kembalian
15. Kasir mengkonfirmasi transaksi dengan menekan tombol "Process Payment"
...
23. Kasir mencetak struk untuk diberikan kepada customer
...
**Tabel 4.14 Spesifikasi Use Case Laporan Penjualan**
...
**Tabel 4.15 Spesifikasi Use Case Kelola Produk**
...
Deskripsi use case merupakan penjelasan rinci mengenai interaksi antara aktor dengan sistem untuk setiap fungsionalitas yang tersedia. Berdasarkan diagram use case yang telah dijelaskan sebelumnya, terdapat tiga aktor yang berinteraksi dengan sistem POS DW, yaitu Admin, Kasir, dan Manager. Berikut adalah deskripsi use case untuk masing-masing aktor.

### Deskripsi Use Case Admin

Admin merupakan aktor dengan hak akses tertinggi dalam sistem yang bertanggung jawab dalam pengelolaan seluruh data dan konfigurasi sistem. Adapun deskripsi use case untuk aktor Admin dapat dilihat pada tabel 4.7 sebagai berikut:

**Tabel 4.10 Deskripsi Use Case Admin**

| Kode | Use Case | Deskripsi |
|------|----------|-----------|
| UC-01 | Login | Admin melakukan login menggunakan email dan password untuk masuk ke dalam sistem |
| UC-02 | Setup 2FA | Admin mengaktifkan atau menonaktifkan fitur Two Factor Authentication |
| UC-03 | Logout | Admin keluar dari sistem dan menghapus session |
| UC-04 | Kelola Kategori | Admin mengelola data kategori produk (Create, Read, Update, Delete) |
| UC-05 | Kelola Produk | Admin mengelola data produk termasuk informasi detail, harga, dan stok |
| UC-06 | Upload Gambar Produk | Admin mengunggah dan mengelola gambar untuk setiap produk |
| UC-07 | Kelola Stok | Admin melakukan penyesuaian stok produk secara manual |
| UC-08 | Transaksi Penjualan | Admin melakukan proses transaksi penjualan POS |
| UC-09 | Lihat Riwayat Transaksi | Admin melihat daftar transaksi yang telah dilakukan |
| UC-10 | Laporan Penjualan | Admin menghasilkan laporan penjualan berdasarkan periode waktu |
| UC-11 | Laporan Stok | Admin menghasilkan laporan stok produk termasuk stok menipis |
| UC-12 | Laporan Produk Terlaris | Admin menghasilkan laporan produk dengan penjualan tertinggi |
| UC-13 | Export PDF | Admin mengekspor laporan ke dalam format PDF |
| UC-14 | Export Excel | Admin mengekspor laporan ke dalam format Excel |
| UC-15 | Kelola User | Admin mengelola data pengguna sistem (CRUD) |
| UC-16 | Kelola Role | Admin mengelola role atau peran pengguna dalam sistem |
| UC-17 | Kelola Permission | Admin mengelola hak akses untuk setiap role |
| UC-18 | Settings Toko | Admin mengatur konfigurasi toko (nama, alamat, logo, telepon) |
| UC-19 | Settings Profil | Admin memperbarui profil pengguna (nama, email) |
| UC-20 | Ubah Password | Admin mengganti password akun pengguna |

*Sumber: Analisis Use Case (2026)*

### Deskripsi Use Case Kasir

Kasir merupakan aktor yang bertugas melayani transaksi penjualan di kasir. Kasir memiliki akses terbatas pada modul transaksi dan data yang mendukung operasional kasir. Adapun deskripsi use case untuk aktor Kasir dapat dilihat pada tabel 4.8 sebagai berikut:

**Tabel 4.11 Deskripsi Use Case Kasir**

| Kode | Use Case | Deskripsi |
|------|----------|-----------|
| UC-01 | Login | Kasir melakukan login menggunakan email dan password untuk masuk ke dalam sistem |
| UC-02 | Setup 2FA | Kasir mengaktifkan atau menonaktifkan fitur Two Factor Authentication |
| UC-03 | Logout | Kasir keluar dari sistem dan menghapus session |
| UC-08 | Transaksi Penjualan | Kasir melakukan proses transaksi penjualan POS |
| UC-09 | Lihat Riwayat Transaksi | Kasir melihat daftar transaksi yang telah dilakukan |
| UC-19 | Settings Profil | Kasir memperbarui profil pengguna (nama, email) |
| UC-20 | Ubah Password | Kasir mengganti password akun pengguna |

*Sumber: Analisis Use Case (2026)*

### Deskripsi Use Case Manager

Manager merupakan aktor yang bertugas melakukan monitoring dan analisis bisnis. Manager membutuhkan data laporan untuk pengambilan keputusan. Adapun deskripsi use case untuk aktor Manager dapat dilihat pada tabel 4.9 sebagai berikut:

**Tabel 4.12 Deskripsi Use Case Manager**

| Kode | Use Case | Deskripsi |
|------|----------|-----------|
| UC-01 | Login | Manager melakukan login menggunakan email dan password untuk masuk ke dalam sistem |
| UC-02 | Setup 2FA | Manager mengaktifkan atau menonaktifkan fitur Two Factor Authentication |
| UC-03 | Logout | Manager keluar dari sistem dan menghapus session |
| UC-09 | Lihat Riwayat Transaksi | Manager melihat daftar transaksi yang telah dilakukan |
| UC-10 | Laporan Penjualan | Manager menghasilkan laporan penjualan berdasarkan periode waktu |
| UC-11 | Laporan Stok | Manager menghasilkan laporan stok produk termasuk stok menipis |
| UC-12 | Laporan Produk Terlaris | Manager menghasilkan laporan produk dengan penjualan tertinggi |
| UC-13 | Export PDF | Manager mengekspor laporan ke dalam format PDF |
| UC-14 | Export Excel | Manager mengekspor laporan ke dalam format Excel |
| UC-19 | Settings Profil | Manager memperbarui profil pengguna (nama, email) |
| UC-20 | Ubah Password | Manager mengganti password akun pengguna |
...
![Activity Login](bab-iv/diagrams/activity-login.png)
...
Activity diagram transaksi penjualan menggambarkan alur aktivitas dari proses transaksi POS yang dilakukan oleh kasir.

### Gambar 4.4 Activity Diagram Transaksi POS

![Activity Transaksi](bab-iv/diagrams/activity-transaksi.png)

*Sumber: Hasil Perancangan (2026)*

### Penjelasan Activity Diagram Transaksi

Activity diagram transaksi penjualan terdiri dari dua swimlane yaitu **Kasir** dan **Sistem**. Diagram ini merupakan yang paling kompleks karena menggambarkan alur bisnis inti dari sistem POS DW.

**Tahap 1: Inisialisasi POS**
- Kasir membuka halaman transaksi POS
- Sistem menampilkan interface POS dengan daftar produk dan menginisialisasi keranjang kosong

**Tahap 2: Pemilihan Produk**
- Kasir mencari produk dengan mengetikkan nama atau SKU
...
- Kasir dapat mengubah quantity dan sistem akan memvalidasi ketersediaan stok
- Sistem menghitung subtotal per item dan total keseluruhan secara real-time

**Tahap 4: Pembayaran**
- Kasir memasukkan nama customer dan memilih metode pembayaran
...
- Kasir mengkonfirmasi transaksi
...
- Kasir mencetak struk untuk customer
...
![Activity Laporan](bab-iv/diagrams/activity-laporan.png)

*Sumber: Hasil Perancangan (2026)*

### Penjelasan Activity Diagram Laporan

Activity diagram laporan terdiri dari swimlane **User (Manager/Admin)** dan **Sistem**. Diagram ini menggambarkan proses pembuatan laporan bisnis yang melibatkan pemilihan jenis laporan, pengaturan filter, hingga export data.
...
![Sequence Login](bab-iv/diagrams/sequence-login.png)
...
Sequence diagram transaksi penjualan menggambarkan interaksi yang terjadi ketika kasir melakukan transaksi POS. Diagram ini melibatkan enam objek: Kasir, Browser, Livewire Component, Transaction Model, Product Model, TransactionItem Model, dan MySQL Database.

### Gambar 4.7 Sequence Diagram Transaksi

![Sequence Transaksi](bab-iv/diagrams/sequence-transaksi.png)
...
Proses dimulai ketika Kasir membuka halaman transaksi POS. Browser mengirimkan request yang me-mount Livewire TransactionComponent. Component ini melakukan query ke Product Model untuk mengambil seluruh produk aktif beserta kategorinya. Data produk dikembalikan dalam bentuk Collection dan di-render menjadi interface POS yang terdiri dari daftar produk dan keranjang belanja.

**2. Pencarian Produk**

Kasir mengetik di kolom pencarian. Fitur wire:model.live dari Livewire mengirimkan setiap perubahan input ke server secara real-time. Component memfilter koleksi produk berdasarkan kata kunci pencarian. Browser memperbarui tampilan daftar produk tanpa reload.

**3. Penambahan Item ke Keranjang**

Kasir mengklik tombol tambah pada produk yang diinginkan. Browser mengirimkan event wire:click yang memanggil method addToCart() dengan parameter product ID. Component melakukan:
...
Ketika Kasir mengklik "Process Payment":
...
Sequence diagram laporan menggambarkan interaksi yang terjadi ketika user (Manager/Admin) membuat laporan bisnis. Diagram ini melibatkan objek: User, Browser, Livewire ReportComponent, Transaction Model, Product Model, PDF Generator, Excel Exporter, dan MySQL Database.

### Gambar 4.8 Sequence Diagram Laporan

![Sequence Laporan](bab-iv/diagrams/sequence-laporan.png)
...
![Class Diagram](bab-iv/diagrams/class-diagram.png)
...
**Tabel 4.16 Deskripsi Class dalam Sistem**
...
**Tabel 4.17 Relasi dan Kardinalitas**
...
Relasi ini menggunakan tabel pivot `model_has_roles` dari package Spatie Laravel Permission. Seorang user dapat memiliki lebih dari satu role (misalnya seorang Admin juga bisa menjadi Manager), dan sebuah role dapat dimiliki oleh banyak user. Relasi ini memungkinkan fleksibilitas dalam pengaturan hak akses.
...
## 4.2.5 Analisis Input dan Output

Analisis input dan output merupakan tahapan untuk mengidentifikasi data-data yang dibutuhkan oleh sistem dan data-data yang dihasilkan oleh sistem. Analisis ini bertujuan untuk memastikan bahwa sistem dapat menerima masukan yang diperlukan dan menghasilkan luaran yang sesuai dengan kebutuhan pengguna.

### 4.2.5.1 Analisis Input

Berdasarkan analisis proses yang telah dilakukan, data input yang dibutuhkan oleh sistem POS DW terdiri dari data master, data transaksional, dan data konfigurasi. Adapun analisis input sistem dapat dilihat pada tabel 4.15 sebagai berikut:

**Tabel 4.18 Analisis Input Sistem POS DW**

| No | Nama Data | Tipe Data | Sumber | Deskripsi |
|----|-----------|-----------|--------|-----------|
| 1 | Data Login | Text (Email, Password) | User | Data autentikasi untuk masuk ke sistem |
| 2 | Data Kategori | Text (Nama, Slug) | Admin | Data untuk mengelola kategori produk |
| 3 | Data Produk | Text, Numeric, File | Admin | Data detail produk termasuk nama, harga, stok, dan gambar |
| 4 | Data Transaksi | Numeric, Text | User | Data transaksi penjualan termasuk item dan pembayaran |
| 5 | Data User | Text, Email, Password | Admin | Data pengguna sistem termasuk kredensial |
| 6 | Data Role | Text | Admin | Data role atau peran dalam sistem |
| 7 | Data Permission | Text | Admin | Data hak akses untuk setiap fitur |
| 8 | Data Settings | Text, File | Admin | Data konfigurasi toko dan profil |
| 9 | Data Password | Text (Password) | User | Data perubahan password pengguna |
| 10 | Data OTP 2FA | Numeric | User | Kode OTP untuk verifikasi Two Factor Authentication |

*Sumber: Analisis Sistem (2026)*

### 4.2.5.2 Analisis Output

Berdasarkan analisis proses sistem, output yang dihasilkan oleh sistem POS DW terdiri dari tampilan informasi, laporan, dan dokumen cetak. Adapun analisis output sistem dapat dilihat pada tabel 4.16 sebagai berikut:

**Tabel 4.19 Analisis Output Sistem POS DW**

| No | Nama Data | Tujuan | Media | Deskripsi |
|----|-----------|--------|-------|-----------|
| 1 | Dashboard Informasi | User | Layar (Browser) | Tampilan ringkasan statistik penjualan, stok, dan aktivitas |
| 2 | Struk Transaksi | Pembeli | Cetak (Printer) | Bukti transaksi penjualan yang dicetak |
| 3 | Riwayat Transaksi | User | Layar (Browser) | Daftar transaksi yang telah dilakukan |
| 4 | Laporan Penjualan PDF | Manager | File PDF | Laporan penjualan dalam format PDF |
| 5 | Laporan Penjualan Excel | Manager | File Excel | Laporan penjualan dalam format Excel |
| 6 | Laporan Stok PDF | Manager | File PDF | Laporan stok produk dalam format PDF |
| 7 | Laporan Stok Excel | Manager | File Excel | Laporan stok produk dalam format Excel |
| 8 | Laporan Produk Terlaris PDF | Manager | File PDF | Laporan produk terlaris dalam format PDF |
| 9 | Laporan Produk Terlaris Excel | Manager | File Excel | Laporan produk terlaris dalam format Excel |
| 10 | Data Statistik Real-time | User | Layar (Browser) | Informasi statistik yang diperbarui secara real-time |

*Sumber: Analisis Sistem (2026)*

---

## Ringkasan Analisis Proses Sistem

Analisis proses sistem yang telah dijelaskan pada sub-bab ini mencakup enam aspek pemodelan yang saling melengkapi:

1. **Use Case Diagram** mengidentifikasi tiga aktor (Admin, Kasir, Manager) dengan 20 use case yang mencakup seluruh fungsionalitas sistem. Setiap use case telah dijelaskan secara detail pada diagram use case.

2. **Deskripsi Use Case** memberikan penjelasan rinci mengenai interaksi masing-masing aktor dengan sistem. Admin memiliki 20 use case, Kasir memiliki 7 use case, dan Manager memiliki 11 use case yang disesuaikan dengan peran dan tanggung jawab masing-masing aktor.
...
6. **Analisis Input dan Output** mengidentifikasi sepuluh data input yang dibutuhkan sistem dan sepuluh data output yang dihasilkan sistem, memastikan kesesuaian data antara yang dimasukkan dan yang dihasilkan.

Seluruh analisis proses ini telah mempertimbangkan aspek keamanan (autentikasi berlapis, proteksi brute force, CSRF), kemudahan penggunaan (interface real-time, validasi interaktif), skalabilitas (struktur database yang ternormalisasi), dan maintainability (pola desain MVC yang terstruktur).

Analisis proses yang telah dibuat akan menjadi acuan dalam tahap perancangan perangkat lunak yang dijelaskan pada sub-bab selanjutnya.

---

## Navigasi

| [← Daftar Isi](./README.md) | [← 4.1 Analisa Kebutuhan Sistem](./4.1-analisis-kebutuhan-sistem.md) | [4.3 Perancangan Basis Data →](./4.3-perancangan-basis-data.md) |
|:---:|:---:|:---:|
